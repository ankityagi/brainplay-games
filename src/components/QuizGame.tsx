...
export default function QuizGame({ questions, timePerQuestion, onFinish, scoreSuffix = '' }: QuizGameProps) {
  ...
  return (
    <div className="flex-1 flex flex-col max-w-2xl mx-auto w-full px-4 py-6 gap-5">
      ...
      {timePerQuestion ? (
        <div className="self-end text-sm font-mono text-slate-400">⏱ {timeLeft}s</div>
      ) : null}

      <div className="flex-1 flex flex-col items-center justify-center gap-6 text-center">
        <div className="text-2xl sm:text-3xl font-bold">{question.prompt}</div>
          ...
          {question.choices.map((choice, i) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl border py-4 px-3 font-semibold text-lg transition-colors ${style}`}
              >
                {choice}
              </button>
            );
        ...
        {feedback === 'timeout' && <p className="text-rose-400 text-sm">Time&apos;s up!</p>}
      </div>
      <p className="text-center text-xs text-slate-600">Pass with {Math.round(PASS_THRESHOLD * 100)}%+ correct</p>
    </div>
  );
}
